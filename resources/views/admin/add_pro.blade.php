...
<div class="x_panel">
	...
	<div class="x_content">
		...
		?>
				...
				<a href="{{route('listing.index',['model'=>'Product'])}}">Retour à la page de liste</a>
		<?php
			} else {
		?>
		<form class="form-horizontal form-label-left" method="post" action="{{route('editing.store',['model'=>'Product'])}}">
			@csrf
			...
			<div class="form-group row ">
				...
				<div class="col-md-9 col-sm-9 ">
					<input type="text" class="form-control" placeholder="Price" name="price" id="price" class="@error('price') is-invalid @enderror">
					@error('price')
					    <div class="alert alert-danger">{{ $message }}</div>
					@enderror
				</div>
